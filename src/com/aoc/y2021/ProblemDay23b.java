...
public class ProblemDay23b implements ProblemDay<Integer> {
    ...
    public Integer solve() {
        ...
        stacks.get('A').add(new Pair<>("B1", new Pair<>(5, 3)));
        stacks.get('A').add(new Pair<>("D1", new Pair<>(4, 3)));
        stacks.get('A').add(new Pair<>("D2", new Pair<>(3, 3)));
        stacks.get('A').add(new Pair<>("B2", new Pair<>(2, 3)));
        stacks.get('B').add(new Pair<>("C1", new Pair<>(5, 5)));
        stacks.get('B').add(new Pair<>("B3", new Pair<>(4, 5)));
        stacks.get('B').add(new Pair<>("C2", new Pair<>(3, 5)));
        stacks.get('B').add(new Pair<>("C3", new Pair<>(2, 5)));
        stacks.get('C').add(new Pair<>("D3", new Pair<>(5, 7)));
        stacks.get('C').add(new Pair<>("A1", new Pair<>(4, 7)));
        stacks.get('C').add(new Pair<>("B4", new Pair<>(3, 7)));
        stacks.get('C').add(new Pair<>("A2", new Pair<>(2, 7)));
        stacks.get('D').add(new Pair<>("A3", new Pair<>(5, 9)));
        stacks.get('D').add(new Pair<>("C4", new Pair<>(4, 9)));
        stacks.get('D').add(new Pair<>("A4", new Pair<>(3, 9)));
        stacks.get('D').add(new Pair<>("D4", new Pair<>(2, 9)));

//        stacks.get('A').add(new Pair<>("A1", new Pair<>(5, 3)));
//        stacks.get('A').add(new Pair<>("D1", new Pair<>(4, 3)));
//        stacks.get('A').add(new Pair<>("D2", new Pair<>(3, 3)));
//        stacks.get('A').add(new Pair<>("B1", new Pair<>(2, 3)));
//        stacks.get('B').add(new Pair<>("D3", new Pair<>(5, 5)));
//        stacks.get('B').add(new Pair<>("B2", new Pair<>(4, 5)));
//        stacks.get('B').add(new Pair<>("C1", new Pair<>(3, 5)));
//        stacks.get('B').add(new Pair<>("C2", new Pair<>(2, 5)));
//        stacks.get('C').add(new Pair<>("C3", new Pair<>(5, 7)));
//        stacks.get('C').add(new Pair<>("A2", new Pair<>(4, 7)));
//        stacks.get('C').add(new Pair<>("B3", new Pair<>(3, 7)));
//        stacks.get('C').add(new Pair<>("B4", new Pair<>(2, 7)));
//        stacks.get('D').add(new Pair<>("A3", new Pair<>(5, 9)));
//        stacks.get('D').add(new Pair<>("C4", new Pair<>(4, 9)));
//        stacks.get('D').add(new Pair<>("A4", new Pair<>(3, 9)));
//        stacks.get('D').add(new Pair<>("D4", new Pair<>(2, 9)));

//        stacks.get('A').add(new Pair<>("B1", new Pair<>(3, 3)));
//        stacks.get('A').add(new Pair<>("B2", new Pair<>(2, 3)));
//        stacks.get('B').add(new Pair<>("C1", new Pair<>(3, 5)));
//        stacks.get('B').add(new Pair<>("C2", new Pair<>(2, 5)));
//        stacks.get('C').add(new Pair<>("D1", new Pair<>(3, 7)));
//        stacks.get('C').add(new Pair<>("A1", new Pair<>(2, 7)));
//        stacks.get('D').add(new Pair<>("A2", new Pair<>(3, 9)));
//        stacks.get('D').add(new Pair<>("D2", new Pair<>(2, 9)));

//        stacks.get('A').add(new Pair<>("A1", new Pair<>(3, 3)));
//        stacks.get('A').add(new Pair<>("B1", new Pair<>(2, 3)));
//        stacks.get('B').add(new Pair<>("D1", new Pair<>(3, 5)));
//        stacks.get('B').add(new Pair<>("C1", new Pair<>(2, 5)));
//        stacks.get('C').add(new Pair<>("C2", new Pair<>(3, 7)));
//        stacks.get('C').add(new Pair<>("B2", new Pair<>(2, 7)));
//        stacks.get('D').add(new Pair<>("A2", new Pair<>(3, 9)));
//        stacks.get('D').add(new Pair<>("D2", new Pair<>(2, 9)));
...
//                .collect(Collectors.toSet());
    ...
    private Set<State> score(final Map<String, Pair<Integer, Integer>> hallwayPositions,
        ...
        for (final Map.Entry<String, Pair<Integer, Integer>> position : moveablePositions.entrySet()) {
            ...
            // hallwayPositions.getOrDefault("B2", new Pair<>(0, 0)).equals(new Pair<>(1, 4))
            // hallwayPositions.getOrDefault("B2", new Pair<>(0, 0)).equals(new Pair<>(1, 4)) && hallwayPositions.getOrDefault("C1", new Pair<>(0, 0)).equals(new Pair<>(1, 6))
            // hallwayPositions.getOrDefault("B2", new Pair<>(0, 0)).equals(new Pair<>(1, 4)) && stacks.get('C').peek().getKey().equals("C1")
            // hallwayPositions.getOrDefault("B2", new Pair<>(0, 0)).equals(new Pair<>(1, 4)) && stacks.get('C').peek().getKey().equals("C1") && hallwayPositions.getOrDefault("D1", new Pair<>(0, 0)).equals(new Pair<>(1, 6))
            ...
            for (final Pair<Integer, Integer> move : validMoves) {
                ...
                final Map<Character, Stack<Pair<String, Pair<Integer, Integer>>>> newStacks = stacks.entrySet().stream()
                                .collect(Collectors.toMap(
                                        ...
                                        e -> e.getValue().stream()
                                                .map(p -> new Pair<>(
                                                        p.getKey(),
                                                        new Pair<>(p.getValue().getKey(), p.getValue().getValue())))
                ...
                if (move.getKey() == 1) {
                    ...
                    newStacks.get(active).add(new Pair<>(position.getKey(), move));
                }
        ...
        if (position.getKey() == 1) {
            ...
            if (isPathOccupied) {
                ...
                return new HashSet<Pair<Integer, Integer>>() {{
                    add(new Pair<>(STACK_LIMIT - stack.size() + 1, TARGET_COLUMN.get(active)));
                }};
            }
        } else {
