...
public class ProblemDay19b implements ProblemDay<Integer> {
    ...
    public Integer solve() {
        ...
        final Set<Beacon> beacons = new TreeSet<>(COMPARATOR);
        final List<List<Integer>> shifts = new ArrayList<>();
        ...
        while (!scanners.isEmpty()) {
            ...
            final Pair<Set<Beacon>, List<Integer>> aligned = align(beacons, s.getBeacons());
            if (aligned.getKey().size() > 0) {
                beacons.addAll(aligned.getKey());
                shifts.add(aligned.getValue());
                System.out.printf("Unique: %d.  Queue size: %d%n", beacons.size(), scanners.size());
            } else {
                scanners.add(s);
            }
        }
        ...
        int max = 0;
        for (final List<Integer> a : shifts) {
            for (final List<Integer> b : shifts) {
                if (a.equals(b)) {
                    continue;
                }
                final int d = Math.abs(a.get(0) - b.get(0)) + Math.abs(a.get(1) - b.get(1)) + Math.abs(a.get(2) - b.get(2));
                if (d > max) {
                    max = d;
                }
            }
    ...
    private Pair<Set<Beacon>, List<Integer>> align(final Set<Beacon> matches, final Set<Beacon> current) {
    ...
    public Scanner getProblemInputStream() throws IOException {
        scanner = new Scanner(Paths.get(".", "src", "com/aoc/y2021/day19.txt"));
        return scanner;
    }
}
