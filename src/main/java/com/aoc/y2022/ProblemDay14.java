...
public class ProblemDay14 extends ProblemDay<Integer, Integer> {
    ...
    public Integer solveA() {
        ...
        int boundary = grid.yRange().upperEndpoint();
        int grains = 0;
        while (addGrainOfSand(grid, boundary, true)) {
            grains++;
        }
        return grains;
    }
    ...
    public Integer solveB() {
        ...
        int grains = 0;
        while (addGrainOfSand(grid, boundary, false)) {
            grains++;
        }
        return grains;
    }

    private boolean addGrainOfSand(SparseMatrix<Character> grid, int boundary, boolean exitAtBoundary) {
        ...
        while (true) {
            ...
            if (exitAtBoundary && down.getRow() > boundary) {
                return false;
            }
